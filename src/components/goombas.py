...
class Goombas(Entity):
    def __init__(self, x_pos, y_pos, move_direction):
        ...
        self.image_tick = 0
        self.images = [
            pygame.image.load('assets/images/goombas_0.png').convert_alpha(),
            pygame.image.load('assets/images/goombas_1.png').convert_alpha(),
            pygame.image.load('assets/images/goombas_dead.png').convert_alpha()
        ]
    ...
    def die(self, core, instantly, crushed):
        if not instantly:
            ...
            core.get_map().spawn_score_text(self.rect.x, self.rect.y)
