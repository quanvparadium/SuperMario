...
class Player(object):
    ...
    def player_physics(self, core):
        if core.keyR:
            self.x_velocity += SPEED_INCREASE_RATE
            self.direction = True
        if core.keyL:
            self.x_velocity -= SPEED_INCREASE_RATE
        ...
        if not (core.keyR or core.keyL):
            if self.x_velocity > 0:
                self.x_velocity -= SPEED_DECREASE_RATE
            elif self.x_velocity < 0:
                self.x_velocity += SPEED_DECREASE_RATE
        else:
            if self.x_velocity > 0:
                if self.fast_moving:
                    if self.x_velocity > MAX_FASTMOVE_SPEED:
                        self.x_velocity = MAX_FASTMOVE_SPEED
                else:
                    if self.x_velocity > MAX_MOVE_SPEED:
                        self.x_velocity = MAX_MOVE_SPEED
            if self.x_velocity < 0:
                if self.fast_moving:
                    if (-self.x_velocity) > MAX_FASTMOVE_SPEED: self.x_velocity = -MAX_FASTMOVE_SPEED
                else:
                    if (-self.x_velocity) > MAX_MOVE_SPEED:
                        self.x_velocity = -MAX_MOVE_SPEED

        # removing the computational error
        if 0 < self.x_velocity < SPEED_DECREASE_RATE:
            self.x_velocity = 0
        if 0 > self.x_velocity > -SPEED_DECREASE_RATE:
            self.x_velocity = 0
        ...
        if not self.on_ground:
            ...
            if (self.y_velocity < 0 and core.keyU):
                self.y_velocity += GRAVITY
                
            # Moving up, button is not pressed - low jump
            elif (self.y_velocity < 0 and not core.keyU):
                self.y_velocity += GRAVITY * LOW_JUMP_MULTIPLIER
            
            # Moving down
            else:
                self.y_velocity += GRAVITY * FALL_MULTIPLIER
    ...
    def update_image(self, core):
        ...
        if self.powerLVL in (0, 1, 2):
            ...
            # Player is running
                    ...
                    ((self.x_velocity > 0 and core.keyR and not core.keyL) or
                     (self.x_velocity < 0 and core.keyL and not core.keyR)) or
                    (self.x_velocity > 0 and not (core.keyL or core.keyR)) or
                    (self.x_velocity < 0 and not (core.keyL or core.keyR))
            ):
            ...
            elif (self.x_velocity > 0 and core.keyL and not core.keyR) or (self.x_velocity < 0 and core.keyR and not core.keyL):
                self.set_image(7)
                self.spriteTick = 0
            ...
            if block != 0 and block.type != 'BGObject':
                if pygame.Rect.colliderect(self.rect, block.rect):
                    ...
                    if self.y_velocity > 0:
                        ...
                        self.y_velocity = -self.y_velocity / 3
    ...
    def flag_animation_move(self, core, walk_to_castle):
        if walk_to_castle:
            ...
            if not self.on_ground:
                self.y_velocity += GRAVITY if self.y_velocity <= MAX_FALL_SPEED else 0
